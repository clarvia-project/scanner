    ...
    <div className="space-y-8">
      ...
      {profile.clarvia_score != null && (
        <div className="space-y-4">
          <h2 className="text-xs font-mono text-accent uppercase tracking-widest">Clarvia Score</h2>
          <div className={`glass-card rounded-2xl px-7 py-8 text-center space-y-4 ${scoreGlowClass(profile.clarvia_score)}`}>
            <span className={`text-6xl font-mono font-bold ${scoreColor(profile.clarvia_score)}`}>
              {profile.clarvia_score}
            </span>
            <span className="text-sm text-muted block font-mono">/ 100</span>
            {profile.rating && (
              <span className={`inline-block px-4 py-1.5 rounded-xl border text-xs font-mono uppercase tracking-wider ${gradeBadgeClass(profile.rating)}`}>
                {profile.rating}
              </span>
            )}
          </div>
      ...
      {profile.dimensions && (
        <div className="space-y-4">
          <h2 className="text-xs font-mono text-accent uppercase tracking-widest">Dimensions</h2>
          <div className="glass-card rounded-2xl px-7 py-6 space-y-5">
            <DimensionBar label="API Accessibility" score={profile.dimensions.api_accessibility.score} max={profile.dimensions.api_accessibility.max} />
            <DimensionBar label="Data Structuring" score={profile.dimensions.data_structuring.score} max={profile.dimensions.data_structuring.max} />
            <DimensionBar label="Agent Compatibility" score={profile.dimensions.agent_compatibility.score} max={profile.dimensions.agent_compatibility.max} />
            <DimensionBar label="Trust Signals" score={profile.dimensions.trust_signals.score} max={profile.dimensions.trust_signals.max} />
          </div>
        </div>
      )}
      ...
      {/* Run Scan */}
      <div className="space-y-4">
        <h2 className="text-xs font-mono text-accent uppercase tracking-widest">Scan</h2>
        <div className="glass-card rounded-2xl px-7 py-6 space-y-4">
          ...
          {scanError && <p className="text-score-red text-xs font-mono">{scanError}</p>}
        </div>
      </div>

      {/* Badge Code */}
      <div className="space-y-4">
        <h2 className="text-xs font-mono text-accent uppercase tracking-widest">Badge</h2>
        <div className="glass-card rounded-2xl px-7 py-6 space-y-4">
          <p className="text-sm text-muted">Add a Clarvia Score badge to your README or website.</p>
          <div className="flex justify-center py-3">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-card-bg border border-card-border">
              <Image src="/logos/clarvia-icon.svg" alt="Clarvia" width={20} height={20} className="rounded-full" />
